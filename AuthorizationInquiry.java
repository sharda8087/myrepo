...
public class AuthorizationInquiry extends BasePage{

	public AuthorizationInquiry(WebDriver driver) {
		super(driver);
	}
	
	@FindBy (xpath = "//button[normalize-space()='Authorization Inquiry']")
    private WebElement AuthInquiryBttn;
	
	@FindBy (xpath = "//h2[starts-with(@id,'modal-heading')]")
    private WebElement AuthHeading;
	...
	public void clickAuthInquiryBttn()
    {
        wait.until(ExpectedConditions.elementToBeClickable(AuthInquiryBttn)).click();

    }
	
	public String ANGHeading()
    {
       return wait.until(ExpectedConditions.visibilityOf(AuthHeading)).getText();
    }
	 ...
	 public String memberName()
	    ...
	    {
	        wait.until(ExpectedConditions.visibilityOf(AuthID)).sendKeys(authId);
	    }
